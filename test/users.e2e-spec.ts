...
describe('UsersController (e2e)', () => {
  ...
  it('Users CRUD', async (done) => {
    ...
    const newUser: UserDTO = { name: 'Test' }
    const newUserRequest = await server.post('/users').type('form').send(newUser).expect(201);
    ...
    const updateUser: UserDTO = { id: newUserRequest.body.id, name: 'Tset' }
